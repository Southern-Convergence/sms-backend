...
export default class App {
  ...
  static async GET_REQUESTS(user_id: ObjectId): Promise<{ data: any, error: any }> {
    ...
    switch (role_name) {
      ...
      case ROLES.EVALUATOR:
        ...
        const EVALUATOR_PENDING = await App.GET_PENDING_EVALUATOR_RO();
        return Promise.resolve({ data: EVALUATOR_PENDING, error: null });
    ...
    return await Database.collection('applicant')?.aggregate([
      {
        $match: {
          $and: [
            { $or: [{ "assignees.5.approved": true }, { $and: [{ "assignees.7.approved": null }, { "assignees.6.evaluator_approved": true }] }] },
            { status: { $in: ["Completed", "For Checking"] } }
  ...
  private static async GET_PENDING_EVALUATOR_RO() {
    return await Database.collection('applicant')?.aggregate([
      {
        $match: {
          $and: [
            { "assignees.6.approved": true },
            { "assignees.7.approved": null },
            { status: "For Evaluation" }
          ]
        }
      },
    ...
    const result = await Database.collection('applicant')?.updateOne({ _id: new ObjectId(app_id) },
      {
        $set: {
          ...
          status: status ? 'Pending' : 'Dissapproved',
